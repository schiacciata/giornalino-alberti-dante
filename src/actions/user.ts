...
import type { userUpdateSchema } from "@/lib/validations/user";
...
export const updateUser = async (values: z.infer<typeof userUpdateSchema>) => {
	...
	values.email = values.email?.toLowerCase().trim() || undefined;
	values.password = values.password?.trim() || undefined;
	values.newPassword = values.newPassword?.trim() || undefined;
	values.isTwoFactorEnabled = values.isTwoFactorEnabled ?? undefined;
...
export const updateUserRole = async (options: { id: string; role: Role }) => {
	const user = await getCurrentUser();
