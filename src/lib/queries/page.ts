...
export async function getPages(postId: string, searchParams: SearchParams) {
	...
	try {
		...
		const orderBy =
			column && order
				? ({ [column]: order } as Record<keyof Page, "asc" | "desc">)
				: null;
		...
		const options = {
			...
			orderBy: orderBy ? [orderBy] : [{ number: "desc" as const }],
		} as any;

		const postPageNumber = Number(number);
		if (!isNaN(postPageNumber) && options.where)
			options.where.number = postPageNumber;
