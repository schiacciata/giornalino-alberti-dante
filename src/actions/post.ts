...
export const likePost = async (formData: postLikeFormData) => {
	...
	return {
		success: body.liked ? t("likes.successLiked") : t("likes.successUnliked"),
	};
};
...
export const editPost = async (formData: FormData) => {
	...
	if (updatedFields.length > 0 && !isAdmin(user))
		return { error: "You can't edit protected fields" };
