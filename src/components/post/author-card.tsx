...
const AuthorCard: FC<AuthorCardProps> = ({ author, publishDate }) => {
    ...
    return (
        <div id='#author' className={cn("flex flex-col gap-2", backdrop, 'p-3 rounded-xl')}>
            <Link href={`/author/${author.id}`} rel="prev" prefetch={false} className="flex items-center gap-2 group">
                <UserAvatar
