...
class Notifications {
	...
	private async handlePushError(subscriptionId: string, err: any) {
		if (err.statusCode === 400) {
			...
			console.log(
				"[webPush] subscription has expired or is no longer valid: ",
				err,
			);
			const deletedSubscripton = await db.pushSubscription.delete({
				where: { id: subscriptionId },
			});
			console.log(
				`[webPush] deleted subscription ${subscriptionId} of user ${deletedSubscripton.userId} due to push error`,
			);
