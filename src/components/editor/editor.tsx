...
type FormData = z.input<typeof pagePatchSchema>;
...
export function Editor({ page, post }: EditorProps) {
	...
	useEffect(() => {
		if (quill === null || socket === null || !localCursors.length) return;
	...
	useEffect(() => {
		...
		return () => {
			...
			quill.off(
				"selection-change",
				selectionChangeHandler(session.user.id || ""),
			);
			if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
		};
		...
		<div className="grid w-full gap-10">
			...
			<div className="prose prose-stone mx-auto w-[800px] dark:prose-invert">
				...
				<input
					...
					{...register("number")}
				/>
				<div id="container" className="min-h-[500px]" ref={wrapperRef} />
			</div>
		</div>
	);
}
